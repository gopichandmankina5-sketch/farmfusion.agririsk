...
def predict_pest_risk(data: dict) -> float:
    ...
    if bundle and "model" in bundle:
        ...
        X = pd.DataFrame([{c: float(data.get(c, 0)) for c in cols}])
        return round(max(0, min(100, float(bundle["model"].predict(X)[0]))), 1)
    # Fallback
    from backend.utils.risk_calculator import calc_pest_risk
    return calc_pest_risk(data["pest_probability"], data["disease_probability"])
...
def predict_yield_risk(data: dict) -> float:
    ...
    return calc_production_risk(data["yield"], base)
...
def predict_market_risk(data: dict) -> float:
    ...
    base = CROP_BASE_PRICE.get(data.get("crop","Rice"), 2500)
    return calc_market_risk(data["avg_market_price"], data["avg_demand"],
                             data["avg_supply"], base)
